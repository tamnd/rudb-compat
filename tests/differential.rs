...
#[test]
fn the_dialect_file_runs_at_fourteen_of_fifteen_through_the_library_driver() {
    let Some(mut duckdb) = duckdb() else { return };
    if !duckdb.is_pinned() {
    ...
    // statement rather than by count so that the day the last one is fixed or one of the other
    // fourteen breaks, it is a diff somebody reviewed rather than a number that quietly moved.
    //
    // The one left is not an engine difference. The slice went with tamnd/rudb#278. `SELECT 1e` is
    // this driver: it wraps the statement in a COPY to get the types out of DuckDB, which puts
    // bytes behind the number, and upstream's tokenizer gives the exponent marker back as soon as
    // there is anything behind it. So DuckDB is asked a question that answers a row and rudb is
    // asked the bare statement, which both engines refuse in the same words since tamnd/rudb#277.
    // Asked the same question through the two shells the file is fifteen of fifteen, and that is
    // the number the corpus header carries. The wrapping is tamnd/rudb-compat#25.
