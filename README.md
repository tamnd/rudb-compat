...
The attach row is 219 files and 4200 of its records are one file, `parallelism/interquery/concurrent_checkpoint_insert.test`, which attaches a hundred databases in a loop, so that row is worth less than the number says and the schema resolution underneath it is worth more.
...
What is left behind a real `require` is mostly one of four things, and all four are now on the engine row with a count: an extension rudb does not have, of which `json`, `icu` and `httpfs` are the largest, a `vector_size` above rudb's 8192, a `block_size`, which rudb has none of because it keeps its tables in memory, and the `tpch` and `tpcds` generators.

The `vector_size` part of that used to be the largest single thing on the row after the extensions, because rudb's vector was 1024 and 136 files ask for 2048. They all run now. Moving 978 records off the engine row cost nothing and bought 198 passes, 369 failures and 391 records on the harness row, and the rate did not move off 33.6 percent, which is the shape to expect when a skip is lifted rather than a feature landed. The 391 is the interesting part: those are things this runner does not do that nothing had reached before, and they are the first files on that row in a while.
...
Each file gets a process of its own, with ten seconds on a statement and two gigabytes on the process. That is not for speed, although it does make the run use every core. It is because the corpus deliberately contains queries that are meant to be enormous, `range(10000000000000000)` and hundred million row cross joins that are there to be stopped. Both limits are handed to the engine, so the normal way one of those ends is rudb raising an error the report can count against the record that asked for it. This process keeps a clock of its own at twelve times the statement limit and a cap on how large the child may get, as a backstop for an engine that does not stop when it is asked to. Twelve, because a file with ten slow statements in it is a file that exists and killing it would throw away the outcome the limits were handed down to produce. A file that reaches one of those is killed and named in the report, and its records are counted in neither column, because a file that was cut off part way through has records nobody has an answer for. Three files have reached one, and the two that still do are named in `corpus/cutoff.txt` with what each of them actually is written beside it. A whole upstream run compares what it killed against that list and prints the difference both ways round, because otherwise the day somebody makes one of them finish is a day a line quietly leaves a report nobody scrolls to, which is what `tamnd/rudb#102` asks for.

It prints and does not fail the build, and that is a measurement rather than a preference. The set is not reproducible. Running the two one at a time, alone, twice each on an idle machine: `overflow/expression_tree_depth.test` finished once and ran past two minutes once, and `catalog/table/create_table_as_abort.test` finished once and reached 2982 MB against the 2048 MB cap once. Same file, same binary, same machine, nothing else running. An exact comparison over a set that moves on its own would be red about half the time with nothing having changed, which is the check-nobody-reads problem again and worse, because this one would be red about something real. So one run disagreeing with the list is not a reason to edit it and a name that has settled down either way is.

The nondeterminism is the more interesting half of what this found, and it is `tamnd/rudb#736` rather than something papered over here. The three names this list has held are also three different problems. `overflow/expression_tree_depth.test` is not slow work at all, it is one `x + x + x` about three thousand terms long, and DuckDB answers it with `Parser Error: Max expression depth limit of 250 exceeded.` after the file sets `max_expression_depth=250`; rudb has no such setting, so instead of refusing the expression it builds the tree and walks it. `catalog/table/create_table_as_abort.test` goes over on memory rather than on the clock, and what it is saying is that rudb builds the whole result before it finds out the statement is going to be rolled back. No amount of planner work reaches either of those.

`optimizer/table_filters.test` was the third and it is the one that has come off. It was the only one that was simply too slow: 88 records of filter statistics reproducers joining build sides against a million rows, with the join order and the build side heuristics turned off by the file itself, so what was left was exactly the work the statistics are supposed to prune. It was cut off at the two minute backstop every time it was run, alone or in the corpus. What it was actually waiting on was the join rather than the statistics. `tamnd/rudb#866` taught the lookup to recognise an equality with a cast around one operand, which is what the binder writes for `ON p.k::INTEGER = b.k`, and `tamnd/rudb#883` gave it a residual predicate so that `ON p.k = b.k AND p.g >= b.g` narrows on the equality and filters on the rest instead of falling to a nested loop over every pair. Both landed and the file now runs to the end in five seconds on the gaming machine. The pruning `#102` describes is still not there, and the shape it describes is still real, but this file is no longer the evidence for it.

Those processes can be spread over machines as well as over cores, which is what `--shard k/n` does. A shard takes every kth file of the sorted list, round robin the way `cargo nextest --partition` does it rather than in contiguous blocks, because the list is sorted by path, so a block is a directory, and `copy` and `aggregate` are not the same amount of work. `--out` writes what the shard found in the form `rudb-compat merge` reads back, and `scripts/shard` is the whole loop over server1, server2, server3 and the gaming machine with the merge at the end. A merged run prints the corpus summary and no page, because a page carries the provenance of the one machine it was measured on and this has four. Every shard writes down the corpus commit it ran over and `merge` refuses a set of shards that do not agree on it, which is not a precaution but a bug that already happened: the corpus is fetched per machine, the upstream ref is a branch that moves, and the first real run of this had two machines ten files apart and merged into a pass rate that was neither of theirs. It also buys nothing today, and that is worth knowing before anybody runs it: the corpus is 177 processor seconds of work over 4140 files, the seven slowest are 157 of the 177, and no number of machines gets below the slowest single file. The run takes 42 seconds on 32 cores. It was 364 seconds over 4106 files and 121 seconds on 32 cores before the join work landed, and the file at the top of that list, at 125 of the 364, was `optimizer/table_filters.test`, which is five seconds now. The two above it today are the two left in `corpus/cutoff.txt`, and neither of them is a join. So this is here because it is fifty lines. Section 9.8 of the harness spec has the measurements and the reason two machines do not currently print the same pass rate.
...
It always drives both engines as shells that remember what they were told, rather than taking `--shell` as a choice. A test file makes a table and then asks questions about it, so a driver that forgets between statements fails every record after the first one for a reason that has nothing to do with the record. The library pair forgets on one side only, because the linked rudb keeps a connection open and the DuckDB driver spawns a fresh in memory process per statement, so every record after the first `CREATE TABLE` passes on rudb and fails on the binary and the whole corpus reads as a harness bug. Two bare shells forget on both sides instead, which cancels out into a stale file rather than a harness bug, and a run where four fifths of the records are the pin failing to find a table it was never told about is a run that measures nothing. Sessions keep one database file and let the engines remember, which is the only arrangement where a whole file means anything and also the only one where what a record costs is what the record costs rather than what the harness costs. A handful of statements cannot live in a file, a temporary table and an `ATTACH` and a transaction among them, and the first of those sends the session back to replaying the statements that left something behind in front of the next one. The committed corpus used to be the exception and replayed throughout, because rudb lost a view when it wrote a file and raised an internal error on a nullable `VARCHAR` read back out of one, which was tamnd/rudb#1264 and tamnd/rudb#1265 and sixteen records between them. Both are fixed, so it is on a database file now like everything else.
